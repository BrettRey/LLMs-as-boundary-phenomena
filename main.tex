...
\begin{abstract}
The debate over whether large language models \enquote{really think} reproduces a familiar pattern: a boundary case meets a binary classification. \textcite{nefdt2026} organises this debate through a $2\times 2$ matrix crossing language with cognition, placing LLMs in a \enquote{missing quadrant} of language without cognition. But the binary frame undersells his own insights. His hedging on proxies, partial capacities, and qualified attributions points toward something the table can't express: that language and cognition are cluster concepts with graded boundaries. \term{Homeostatic property cluster} (HPC) theory and projection-purpose analysis offer a better framework. The same LLM capacities get categorised as \enquote{linguistic} or \enquote{cognitive} depending on the analytical purpose~-- the same phenomenon that makes a tomato a vegetable or a fruit depending on whether you're a greengrocer or a botanist.
\end{abstract}
...
This paper argues that \term{homeostatic property cluster} (HPC) theory \citep{boyd1991, boyd1999} handles the LLM case better. HPC theory deals with kinds defined not by necessary and sufficient conditions but by clusters of co-occurring properties held together by causal mechanisms. It also explains why the debate is so persistent: the answer depends on what the categorisation is for. Nefdt's hedging is a feature, not a bug.

\section{The binary and its discontents}

Nefdt's Table~1 organises three cells of a $2\times 2$ matrix:
...
\bottomrule
\end{tabular}
\caption{Conceptual possibilities, reproduced from \textcite{nefdt2026}, p.~4.}
\label{tab:nefdt}
\end{table}

The open question is whether anything occupies the third row~-- language without cognition. Nefdt's answer is that LLMs fill it. They \enquote{occupy a hitherto vacant part of conceptual space} (p.~4).
...
The same LLM capacities get categorised as \enquote{linguistic} or \enquote{cognitive} depending on the \term{projection purpose}~-- the analytical frame that determines what counts as a member of the category. Consider three projections.

Under a neuroscience projection, we ask what mechanism produces the behaviour. LLMs process tokenised text via vector operations and statistical pattern-matching. By this criterion, their capacities are linguistic: produced by something analogous to a language-processing system, not by the full suite of cognitive machinery.
...
The persistence of the \enquote{do LLMs really think?} debate is predicted by the HPC framework. It's a projection-purpose dispute disguised as a factual one. Resolving it doesn't require discovering a hidden fact about LLMs. It requires specifying the purpose of the categorisation.

\section{Why this matters}

The LLM case isn't just applied philosophy of AI. It's a publicly accessible illustration of how projection purposes shape categorisation~-- a point that's harder to see in technical cases like grammaticality, countability, or the mass/count distinction.
